			...
			if (valor >= 0 && valor <= 7) {
				...
				case 6:
					...
					if (opcao == 1) {
						...
						List<DadosProduto> produtosEstqForn = Estoque.listaProdutosEstoque().stream()
								.filter(x -> x.getFornecedor().equals(fornecedor) && x.getArmazenamentoAtual().equals(Local.ESTOQUE)).collect(Collectors.toList());
				...
				case 7:
					...
					if (opcao == 1) {
						...
						String cpfCliente = sc.next();
						Cliente cl = new Cliente(cpfCliente);
						List<DadosProduto> produtosEstqCliente = Estoque.buscarClientEquals(cl).stream().filter(x -> x.getArmazenamentoAtual().equals(Local.DESPACHE)).collect(Collectors.toList());
						produtosEstqCliente.forEach(System.out::println);
					} else if (opcao == 2) {
						System.out.println("Listando produtos do fornecedor");
						System.out.println("Informe o e-mail do fornecedor");
						...
						List<DadosProduto> produtosEstqForn = EstoqueDespache.listaProdutosDespache().stream()
								.filter(x -> x.getFornecedor().equals(fornecedor) && x.getArmazenamentoAtual().equals(Local.DESPACHE)).collect(Collectors.toList());
						produtosEstqForn.forEach(System.out::println);
					}
					sc.nextLine();
					sc.nextLine(); // Consumir a nova linha pendente
